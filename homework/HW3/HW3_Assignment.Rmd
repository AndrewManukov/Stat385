...
dataset <- WPP2010[WPP2010[, 1] %in% countries, ] #obtain pop for restrited 
dataset[, 3] <- as.numeric(levels(dataset[, 3]))[dataset[, 3]]
dataset[, 4] <- as.numeric(levels(dataset[, 4]))[dataset[, 4]]
dataset[, 3:4] <- dataset[, 3:4] / 1000 #divid by 1000 for clarity
...
```{r}
proportion = length(which(as.numeric(dataset$year) < 1970))
proportion
#FIX
```
...
head(CSCO_Bookings, n = 5) %>% select(EMPLOYMENT, PRISONER.TYPE, MARITIAL.STATUS, BOOKING.TIME, Age.at.Arrest, Days.in.Jail)
```
